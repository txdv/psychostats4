...
class Psychostats_Method_Get_Map_Stats extends Psychostats_Method {
	public function execute($criteria = array(), $gametype = null, $modtype = null) {
		...
		if ($meth = $this->ps->load_overloaded_method('get_map_stats_sql', $gametype, $modtype)) {
			$meth->execute($stats);
		}
		...
		$sql = "SELECT $fields FROM $t_map_data d WHERE mapid = ? LIMIT 1";
