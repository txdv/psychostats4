...
if (defined("PSFILE_FORMS_PHP")) return 1;
define("PSFILE_FORMS_PHP", 1);
...
function val_email($value, &$field) {
  ...
  if (!preg_match($pattern, $value)) $field['error'] = $ps_lang->trans("email address is not valid");
}
...
function val_number($value, &$field) {
  ...
  if ($field['error']) return 1;
  if ($value == '') return 1;
  if (!is_numeric($value)) $field['error'] = $ps_lang->trans("must be a number");
}
...
function val_date($value, &$field) {
  global $ps_lang;
  if ($field['error']) return 1;
  $err = $ps_lang->trans("invalid date specified");
...
function val_blank($value, &$field) {
  global $ps_lang;
  if ($field['error']) return 1;
  if ($value == '') $field['error'] = $ps_lang->trans("can not be blank");
}
