<?php
/**
 * PsychoStats method get_clan_roles()
 * $Id$
 *
 */
class Psychostats_Method_Get_Clan_Roles extends Psychostats_Method {
	/**
	 * Fetches a list of role stats for a specific clan.
	 * 
	...
	public function execute($criteria = array(), $gametype = null, $modtype = null) {
		...
		$c_plr_roles = $this->ps->tbl('c_plr_roles', $gametype, $modtype);
...
<<<CMD
		SELECT $fields
		FROM ($c_plr_roles d, $t_plr plr, $t_role role)
		WHERE plr.plrid=d.plrid AND d.roleid=role.roleid AND plr.clanid=?
CMD;
	...
	protected function get_sql() {
		...
		$t_plr_roles = $this->ps->tbl('c_plr_roles', $this->ps->gametype(), $this->ps->modtype());
		...
		$cols = $this->ps->get_columns($t_plr_roles, false, array( 'plrid','roleid' ));
