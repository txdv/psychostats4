...
/**
 * Smarty {weaponimg} function plugin
 *
 * Type:     function<br>
 * Name:     weaponimg<br>
 * Purpose:  returns the <img> tag for the weapon name, 
		or just a text string of the weapon name/desc if the image file doesn't exist
 * @version  1.0
 * @param array
 * @param Smarty
 */
function smarty_function_weaponimg($args, &$smarty) {
	...
	$args += array(
		'var'		=> '',
		'weapon'	=> array(),	// $weapon array (or weapon name string)
		'alt'		=> NULL,
		'height'	=> NULL,
		'width'		=> NULL,
	...
	$basedir = catfile($ps->conf['theme']['rootweaponsdir'], $ps->conf['main']['gametype'], $ps->conf['main']['modtype']);
	$baseurl = catfile($ps->conf['theme']['rootweaponsurl'], $ps->conf['main']['gametype'], $ps->conf['main']['modtype']);
	if ($args['path']) {
