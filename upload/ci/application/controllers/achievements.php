...
class Achievements extends MY_Controller {
	...
	function index()
	{
		...
		// determine the total achievements available
		$total_achievements = $this->ps->get_total_achievements();
		...
		$achievements = $this->ps->get_achievements($criteria);

		$table = $this->psychotable->create()
			->set_data($achievements)
			...
			;
		$this->ps->mod_table($table, 'achievements', $this->get['gametype'], $this->get['modtype']);
		...
		$page_subtitle = trans('<strong>%s</strong> achievements have been played by <strong>%s</strong> players',
			number_format($total_achievements),
			number_format($total_players)
		);
		$data = array(
			'title'		=> trans('Player Achievements'),
			'page_title' 	=> trans('Player Achievements'),
